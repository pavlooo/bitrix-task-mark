...
class TasksTaskListComponent extends TasksBaseComponent
{
	...
	{
		$stateInstance = Filter\Task::getListStateInstance();
	...
	protected function checkParameters()
	{
		...
		Filter\Task::setUserId($this->arParams["USER_ID"]);
		Filter\Task::setGroupId($this->arParams["GROUP_ID"]);

		$this->setRoleFilter();

		static::tryParseStringParameter($arParams['FILTER_ID'], Filter\Task::getFilterId());
		static::tryParseStringParameter($arParams['GRID_ID'], Filter\Task::getFilterId());

		static::tryParseStringParameter($arParams['NEED_GROUP_BY_GROUPS'], $this->needGroupByGroups() ? 'Y' : 'N');
		static::tryParseStringParameter($arParams['NEED_GROUP_BY_SUBTASKS'], $this->needGroupBySubTasks() ? 'Y' : 'N');

	}

	private function needGroupByGroups()
	{
		$submodes = \CTaskListState::getInstance(\Bitrix\Tasks\Util\User::getId())->getSubmodes();

		return $this->arParams['GROUP_ID'] == 0 && $submodes['VIEW_SUBMODE_WITH_GROUPS']['SELECTED'] == 'Y';
	}

	private function needGroupBySubTasks()
	{
		$instance = \CTaskListState::getInstance(\Bitrix\Tasks\Util\User::getId());
		$submodes = $instance->getSubmodes();
	...
	protected function doPreAction()
	{
		...
		$this->arResult["FILTER"] = Filter\Task::getFilters();
		$this->arResult["PRESETS"] = Filter\Task::getPresets();
	...
	{
		$arrFilter = Filter\Task::processFilter(!$this->arParams['IS_MOBILE']);

		if (array_key_exists('GROUP_ID', $this->arParams) && (int)$this->arParams["GROUP_ID"] > 0)
		{
			$arrFilter['GROUP_ID'] = $this->arParams["GROUP_ID"];
		}
		...
		if ($this->needGroupBySubTasks())
		{
			...
			if (\Bitrix\Main\Grid\Context::isInternalRequest() &&
				check_bitrix_sessid() &&
				$_REQUEST['action'] == Grid\Actions::GRID_GET_CHILD_ROWS
			)
			{
				if(!empty($_REQUEST['parent_id']))
				{
					$arrFilter['PARENT_ID'] = $_REQUEST['parent_id'];
				}
				unset($arrFilter['ONLY_ROOT_TASKS']); // HACK
			}
			else
			{
				$expandedIds = Filter\Task::getGridOptions()->getExpandedRows();

				if ($expandedIds)
				{
					$arrFilter['META:PARENT_ID_OR_NULL'] = array_unique(array_map(function($expandedId){
						if(strpos($expandedId, 'group_') === false)
						{
							return $expandedId;
						}
					}, $expandedIds));
					if(empty($arrFilter['META:PARENT_ID_OR_NULL']))
					{
						unset($arrFilter['META:PARENT_ID_OR_NULL']);
					}
					unset($arrFilter['ONLY_ROOT_TASKS']); // HACK
				}
			}
		...
		$this->listParameters['legacyFilter'] = $arrFilter;
	}
	...
	protected function processGroupActions()
	{
		...
		if ($allTasks)
		{
			...
			$getListParameters = array(
				'select'       => array('ID'),
				'legacyFilter' => array_merge($this->listParameters['legacyFilter'], array('ONLY_ROOT_TASKS'=>'N'))
			);
	...
	private function setRoleFilter()
	{
		$this->arParams['MARK_ACTIVE_ROLE'] = 'Y';
		$this->arParams['MARK_SECTION_ALL'] = 'N';

		$state = Filter\Task::getListStateInstance()->getState();

		if ($state['SECTION_SELECTED']['CODENAME'] === 'VIEW_SECTION_ADVANCED_FILTER')
		{
			Filter\Task::getListStateInstance()->setUserRole(\CTaskListState::VIEW_ROLE_RESPONSIBLE);
			$this->arParams['MARK_SECTION_ALL'] = 'Y';
			$this->arParams['MARK_ACTIVE_ROLE'] = 'N';
		}
		...
		foreach ($state['SPECIAL_PRESETS'] as $preset)
		{
			...
			{
				Filter\Task::getListStateInstance()->setUserRole(\CTaskListState::VIEW_ROLE_RESPONSIBLE);
				$this->arParams['MARK_SPECIAL_PRESET'] = 'Y';
				$this->arParams['MARK_SECTION_ALL'] = 'N';
				$this->arParams['MARK_ACTIVE_ROLE'] = 'N';
				break;
			}
	...
	protected function doPostAction()
	{
		...
		$this->arParams['VIEW_STATE'] = Filter\Task::getListStateInstance()->getState();
	...
	{
		$columns = Filter\Task::getVisibleColumns();
	...
	protected function getOrder()
	{
		...
		)
		{
			...
			Filter\Task::getGridOptions()->setSorting($request->get('SORTF'), $sortResult[$request->get('SORTF')]);
			Filter\Task::getGridOptions()->save();
		}
		else
		{
			$gridSort = Filter\Task::getGridOptions()->GetSorting(
				array(
	...
	{
		Filter\Task::getGridOptions()->resetExpandedRows();
	...
	{
		$navParams = Filter\Task::getGridOptions()->getNavParams(array('nPageSize'=>50));
	...
	private function processSubTaskCounters()
	{
		...
		if ($this->needGroupBySubTasks())
		{
			...
			if (!empty($taskIds))
			{
				$params = $this->listParameters['legacyFilter'];
