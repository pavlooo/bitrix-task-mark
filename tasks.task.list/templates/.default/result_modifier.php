...
if (!function_exists('prepareTaskRowActions'))
{
	function prepareTaskRowActions($row, $arParams)
	{
		...
		$urlPath = $arParams['GROUP_ID'] > 0 ? $arParams['PATH_TO_GROUP_TASKS_TASK'] : $arParams['PATH_TO_USER_TASKS_TASK'];
...
if(!function_exists('tasksCheckRightEditPlan'))
{
	function tasksCheckRightEditPlan($row)
	{
		...
		if (
			(
				($row["RESPONSIBLE_ID"] == Util\User::getId())
				|| (in_array(Util\User::getId(), $row['ACCOMPLICES']))
			)
	...
	function prepareTaskRowTitle($row, $arParams)
	{
		...
		if ((int)$updatesCount)
		{
			$append .= '<a href="'.
				...
				' ('.
				(int)$updatesCount.
				')"><span class="task-item-updates-inner">'.(int)$updatesCount.'</span></a>';
		}
		...
		$title = '<a href="'.$taskUrl.'" class="task-title '.$cssClass.'">'
...
if (!function_exists('prepareTimeTracking'))
{
	function prepareTimeTracking($row, $arParams)
	{
		...
		$currentTaskTimerRunForUser =
			($arParams['TIMER'] !== false)
			&& ($arParams['TIMER']['TASK_ID'] == $row['ID']);

		$taskTimersTotalValue =
			$arParams['TIMER'] && $arParams['TIMER']['RUN_TIME'] && ($currentTaskTimerRunForUser)
...
if (!function_exists('prepareTaskRow'))
{
	...
	{
		$originator = \Bitrix\Tasks\UI\Task::getUserInfo($row['CREATED_BY'], true);
		$responsible = \Bitrix\Tasks\UI\Task::getUserInfo($row['RESPONSIBLE_ID'], true);
			...
			array(
				'ONCHANGE' => array(
					array(
						...
						'DATA'   => array(
							array(
								'JS' => 'BX.Tasks.GridActions.confirmGroupAction(\''.
										$arParams['GRID_ID'].
										'\')'
							)
						)
		...
		if ($canChange)
		{
			return '<span class="task-deadline-datetime"><span class="task-deadline-date"
						onclick="BX.Tasks.GridActions.onDeadlineChangeClick('
				   		.(int)$taskId.', this, \''.$deadline.'\');event.stopPropagation();">'
						.$str.
					'</span></span>';
		}
		else
...
if (!function_exists('prepareTaskRowUserBaloonHtml'))
{
	function prepareTaskRowUserBaloonHtml($arParams)
	{
		...
		$userIcon = '';
		if($arParams['USER']['IS_EXTERNAL'])
			$userIcon = 'tasks-grid-avatar-extranet';
		if ($arParams['USER']["EXTERNAL_AUTH_ID"] == 'email')
			$userIcon = 'tasks-grid-avatar-mail';
		if ($arParams['USER']["IS_CRM"])
			$userIcon = 'tasks-grid-avatar-crm';

		$userAvatar = 'tasks-grid-avatar-empty';
		if($arParams['USER']['AVATAR'])
			$userAvatar = '';


		$userName = '<span class="tasks-grid-avatar  '.$userAvatar.' '.$userIcon.'" 
			'.
			($arParams['USER']['AVATAR'] ? 'style="background-image: url(\''.$arParams['USER']['AVATAR'].'\')"':'').
		'></span>';

		$userName .= '<span class="tasks-grid-username-inner '.$userIcon.'">'.$arParams['USER']['NAME'].'</span>';

		$profilePath = isset($arParams['USER_PROFILE_URL']) ? $arParams['USER_PROFILE_URL'] : '';

		return '<div class="tasks-grid-username-wrapper"><a href="'.htmlspecialcharsbx($profilePath).
			'" class="tasks-grid-username">'.$userName.'</a></div>';
	}
}
		...
		if ($arParams['NEED_GROUP_BY_GROUPS'] === 'Y' && $prevGroupId != $row['GROUP_ID'])
		{
			...
			$groupItem = array(
				...
				"attrs" => array(
					...
					"data-can-edit-tasks" =>
						SocialNetwork\Group::can($row["GROUP_ID"], SocialNetwork\Group::ACTION_EDIT_TASKS) ? "true" : "false"
				)
			);
